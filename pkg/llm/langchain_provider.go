...
// hasCost проверяет, содержит ли usage достоверные данные о стоимости вызова.
...
// cloneModelPrices создает копию pricing map, чтобы избежать внешних мутаций.
...
// enrichUsageWithCost дополняет usage данными о стоимости из провайдера или локальной таблицы цен.
...
// maxInt возвращает v, но не меньше minValue.
...
// setLangfuseGenerationStaticAttributes пишет неизменяемые атрибуты генерации в активный span.
...
// setLangfuseGenerationUsageAndCostAttributes записывает usage/cost атрибуты генерации.
...
// setLangfuseCompletionStartTime фиксирует момент первого токена ответа модели.
...
// providerUsageCandidates извлекает возможные блоки usage/cost из сырого ответа провайдера.
...
// mapFromAny безопасно приводит произвольное значение к map[string]any.
...
// mergeUsageCostFromCandidates выбирает и объединяет usage/cost из списка кандидатов.
...
// costFromMap извлекает cost-поля из карты провайдера.
...
// maxFloat возвращает v, но не меньше minValue.
...
// parseFloat пытается извлечь float64 из разных числовых представлений.
