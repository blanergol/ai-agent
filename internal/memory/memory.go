...
func (m *Manager) ShortTermSnapshot() []llm.Message {
	if m == nil || m.shortTerm == nil {
		return nil
	}
	return m.shortTerm.Messages()
}

// RestoreShortTerm восстанавливает кратковременный контекст из снапшота.
func (m *Manager) RestoreShortTerm(messages []llm.Message) {
	if m == nil || m.shortTerm == nil {
		return
	}
	m.shortTerm.Replace(messages)
}
...
func (m *Manager) BuildContext(ctx context.Context, userInput string) ([]llm.Message, error) {
